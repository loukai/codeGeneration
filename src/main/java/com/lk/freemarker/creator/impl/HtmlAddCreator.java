 ...
 * @author loukai
 */
public class HtmlAddCreator extends AbstractFileCreator {
    private static HtmlAddCreator creator;

    private HtmlAddCreator() {
        super();
    }

    private HtmlAddCreator(Conf conf) {
        super();
        init(conf);
    }

    public static synchronized HtmlAddCreator getInstance(Conf conf) {
        if (null == creator) {
            creator = new HtmlAddCreator(conf);
        }
        return creator;
    }
    ...
    public void createFile(TableInfo tableInfo) throws IOException, TemplateException {
        String ftl = "htmlAdd.ftl";
        String fileName = "add" + tableInfo.getBeanName()+ ".vue";
